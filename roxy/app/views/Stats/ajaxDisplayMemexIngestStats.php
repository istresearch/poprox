...
if ($theRow['total_day_count']>0 && !$v->bUseImportTimes) {
	...
	$jsCode .= Widgets::cnvUtcTs2LocalStr($theTsId, $theRow['first_importtime']);
	$r .= '<span id="'.$theTsId.'">'.$theRow['first_importtime'].'</span>';

	$r .= ' - ';
	
	$theTsId = Strings::createUUID();
	$jsCode .= Widgets::cnvUtcTs2LocalStr($theTsId, $theRow['last_importtime']);
	$r .= '<span id="'.$theTsId.'">'.$theRow['last_importtime'].'</span>';
...
$r .= '<th class="stat-col">Queue Depth</th>';
$w .= $r."</tr>\n";
...
$r .= '<td class="stat-col">'.number_format($theRow['total_ads_01']).'</td>';
$r .= '<td class="stat-col">'.number_format($theRow['queue_depth']).'</td>';
$w .= $r."</tr>\n";
...
if ($theRow['total_day_count']>0) {
	...
	$r .= '<td class="stat-col">'.number_format($theRow['total_ads_01']/1,2).'</td>';
	$r .= '<td class="stat-col"></td>';
	$w .= $r."</tr>\n";
}
